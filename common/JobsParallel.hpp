...
public:
    ...
    /**
     * Число элементов в рабочем массиве. Это и есть пространство итераций работы:
     * именно по нему TBB режет её на части в parallel_for.
     */
    virtual long getScaledSize() = 0;

    /**
     * ОДНА итерация вычисления на ПОДДИАПАЗОНЕ [begin, end). Разбиение по диапазону
     * данных — естественный способ распараллелить векторную операцию: массив один,
     * выделяется один раз, а сколько потоков возьмётся за его части, решает рантайм.
     * Параметр iteration нужен типам, у которых поведение зависит от номера итерации
     * (COPY меняет направление копирования).
     */
    virtual void executeRange(long begin, long end, int iteration) = 0;

    /** Сколько итераций составляют полную работу (как в execute). */
...
public:
    ...
    long getScaledSize() { return size; }

    void executeRange(long begin, long end, int iteration) {
        vdErf((int)(end - begin), x + begin, y + begin);
    }
...
public:
    ...
    void executeRange(long begin, long end, int iteration) {
        vdTGamma((int)(end - begin), x + begin, y + begin);
    }
...
public:
    ...
    void executeRange(long begin, long end, int iteration) {
        vdPow((int)(end - begin), a + begin, b + begin, y + begin);
    }
...
public:
 ...
 //            }
  ...
  //      GLOBAL_EXECUTION_FLAG = changeFlagTo;
    ...
    void executeRange(long begin, long end, int iteration) {
        vdSqrt((int)(end - begin), x + begin, x + begin);
    }
...
public:
  ...
  //          }
    ...
    void executeRange(long begin, long end, int iteration) {
        if (iteration % 2 == 0) {
            cblas_dcopy((int)(end - begin), x + begin, 1, y + begin, 1);
        } else {
            cblas_dcopy((int)(end - begin), y + begin, 1, x + begin, 1);
        }
    }
...
private:
    double* x;
    volatile double dummy_sum = 0.0;   // volatile предотвратит вырезание компилятором
...
public:
 ...
 //       GLOBAL_EXECUTION_FLAG = changeFlagTo;
    ...
    void executeRange(long begin, long end, int iteration) {
        dummy_sum = cblas_dasum((int)(end - begin), x + begin, 1);
    }
...
public:
    ...
    void executeRange(long begin, long end, int iteration) {
        cblas_daxpy((int)(end - begin), 3.5, x + begin, 1, y + begin, 1);
    }
...
public:
 ...
 //            }
  ...
  //      GLOBAL_EXECUTION_FLAG = changeFlagTo;
    ...
    void executeRange(long begin, long end, int iteration) {
        dummy_res = cblas_ddot((int)(end - begin), x + begin, 1, y + begin, 1);
    }
