...
def predict_dieaware(work_type, own_bus, own_cores_str, neighbors):
    ...
    if not own_cores_str:
        # нет данных о раскладке -> откат к простому суммированию
        pressure = own_bus + sum(n_bus for n_bus, _ in neighbors)
    else:
        ...
        for n_bus, n_cores in neighbors:
            ...
            for c in n_cores.split(','):
                ...
                pressure += w * per_core
    return 1.0 + G_DIE[work_type] * max(0.0, pressure / C_DIE - 1.0)
...
def main():
    ...
    print(f"{'модель':<14}{'MAE':>8}{'медиана отн.ошибки':>22}{'доля в ±0.2':>14}")
    print(f"{'простое':<14}{mae('err_simple'):>8.3f}"
          f"{relmed('slowdown_pred_simple')*100:>20.1f}%{within('err_simple'):>13.0f}%")
    print(f"{'die-aware':<14}{mae('err_dieaware'):>8.3f}"
          f"{relmed('slowdown_pred_dieaware')*100:>20.1f}%{within('err_dieaware'):>13.0f}%")

    # --- превью первых конфигураций ---
    print("\n=== Превью (первые 12 работ) ===")
    print(f"{'конфигурация':<28}{'работа':<14}{'Σшина':>6}{'факт':>7}{'простое':>9}{'die-aware':>11}")
    for r in rows[:12]:
        jobs_short = (r['jobs'][:25] + '..') if len(r['jobs']) > 27 else r['jobs']
        print(f"{jobs_short:<28}{r['work']:<14}{r['sum_bus']:>6.0f}"
              f"{r['slowdown_actual']:>7.2f}{r['slowdown_pred_simple']:>9.2f}"
              f"{r['slowdown_pred_dieaware']:>11.2f}")
